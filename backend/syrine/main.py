...
@app.get("/donnees/by_department/{dpt}")
def get_by_department(dpt: str, source: str = "mongo"):
    data = lire_csv() if source == "csv" else list(collection.find({}, {"_id": 0}))
    results = [d for d in data if d.get("DPT", "").lower() == dpt.lower()]
    return results or {"error": "Aucun lycée trouvé dans ce département"}

#  Recherche par ville
@app.get("/donnees/by_city/{ville}")
def get_by_city(ville: str, source: str = "mongo"):
    data = lire_csv() if source == "csv" else list(collection.find({}, {"_id": 0}))
    results = [d for d in data if d.get("OR_VILLE", "").lower() == ville.lower()]
    return results or {"error": "Aucun lycée trouvé dans cette ville"}
